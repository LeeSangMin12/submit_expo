...
const Edit_assignment = ({ navigation, route }) => {
    ...
    const fetch_data = async () => {
      ...
      if (assignment_info.status === 'LMS') {
        ...
        set_assignment_lms_input(() => ({
          url: assignment_submit_info.url,
        ...
        set_assignment_email_input(() => ({
          ...
          title: assignment_submit_info.title,
          description: assignment_submit_info.description,
          file_list: assignment_submit_info.file_list,
        }))
      } else {  //status가 설정일때
        set_submit_method('E-mail');
  ...
  /**
   * submit_assignmet에서 assignment_status을 받아올때만 업데이트
   */
  useEffect(() => {
